    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope, default_name="bert"):
            with tf.variable_scope("embeddings"):
                # Perform embedding lookup on the word ids, but use stype of factorized embedding parameterization from albert
            ...
            with tf.variable_scope("encoder"):
                ...
                if ln_type=='postln' or ln_type is None: # currently, base or large of albert used post-LN structure
                        ...
                        self.all_encoder_layers, self.attention_final = transformer_model(
                                input_tensor=self.embedding_output,
...
def attention_layer(from_tensor,
    ...
    # char attention
    char_attention = tf.reduce_mean(attention_probs,1)
    attention_final = tf.squeeze(char_attention[:,0:1,:], axis=1)
    ...
    return context_layer, attention_final
...
def transformer_model(input_tensor,
        ...
        with tf.variable_scope(name_variable_scope, reuse=True if (share_parameter_across_layers and layer_idx>0) else False):
            ...
            with tf.variable_scope("attention"):
                ...
                with tf.variable_scope("self"):
                    attention_head, attention_final = attention_layer(
    ...
    if do_return_all_layers:
        ...
        return final_outputs, attention_final
    else:
        final_output = reshape_from_matrix(prev_output, input_shape)
        return final_output, attention_final
