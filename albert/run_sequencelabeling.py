...
def convert_single_example(ex_index, example, label_map, label_cid3_map,max_seq_length,
    ...
    if len(label_cid3_ids) > max_seq_length:
        label_cid3_ids = label_cid3_ids[:max_seq_length]
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    loss, per_example_loss, logits, logits_cid3, acc, pred_label_ids, probabilities, predictions, char_weight, precision, recall, f1 = inference(model, num_labels, num_cid3_labels, is_training, labels, labels_cid3, input_mask)

    return (loss, per_example_loss, logits, logits_cid3, pred_label_ids, probabilities, acc, predictions, char_weight,  precision, recall, f1)
...
def inference(model, num_labels, num_cid3_labels, is_training, labels, labels_cid3, input_mask, use_crf=True, use_rnn=False):
    ...
    with tf.variable_scope("loss"):
        ...
        # for char attention
        char_weight = tf.identity(model.attention_final, name='char_weight') 
        # for tag 
        correct_pred = tf.equal(labels, pred_label_ids)
        # remove pad
        mask = tf.cast(input_mask, dtype=tf.float32)
        correct_pred = tf.cast(correct_pred, tf.float32)
        correct_pred = correct_pred * mask
        total_size = tf.reduce_sum(mask) + 1e-12
        acc = tf.reduce_sum(correct_pred) / total_size
    ...
    return (loss, per_example_loss, logits, logits_cid3, acc, pred_label_ids, probabilities, predictions, char_weight, precision, recall, f1)
